...
public class MultiLLMConfig {
	...
	public DashScopeEmbeddingModel dashScopeEmbeddingModel() {
		return DashScopeEmbeddingModel.builder().apiKey(apiKey).model("text-embedding-v3").build();
	}
	...
	public DashScopeMultimodalModel dashScopeMultimodalModel() {
		return DashScopeMultimodalModel.builder().apiKey(apiKey).model("qwen-vl-max").build();
	}
