...
public class MathTaskController {
    ...
    public ResponseEntity<MathTask> createTask(@RequestBody CreateTaskRequest request) {
        ...
        MathTask savedTask = mathTaskService.saveTask(task);
    ...
    public ResponseEntity<Map<String, Object>> executeTask(@PathVariable Long taskId) {
        ...
        try {
            MathTask task = mathTaskService.findById(taskId)
                    .orElseThrow(() -> new RuntimeException("任务不存在"));
            
            // 更新任务状态
            task.setStatus(MathTask.TaskStatus.ANALYZING);
            task.setStartedAt(LocalDateTime.now());
            mathTaskService.saveTask(task);
            
            // 记录开始日志
            mathTaskService.addLog(task, TaskLog.LogLevel.INFO, "GRAPH_EXECUTOR", 
                    "开始执行数学建模Graph工作流", null);
    ...
    public ResponseEntity<List<MathTask>> getTasks() {
        List<MathTask> tasks = mathTaskService.findAll();
    ...
    public ResponseEntity<MathTask> getTask(@PathVariable Long taskId) {
        Optional<MathTask> task = mathTaskService.findById(taskId);
    ...
    public ResponseEntity<Map<String, Object>> cancelTask(@PathVariable Long taskId) {
        try {
            ...
            mathTaskService.addLog(task, TaskLog.LogLevel.INFO, "USER", 
                    "任务已被用户取消", null);
    ...
    private String executeGraphWorkflow(MathTask task) {
        try {
            ...
            Map<String, Object> input = Map.of(
                INPUT_KEY, task.getProblemStatement(),
                "task_id", task.getId(),
            ...
            if (result.isPresent()) {
                ...
                TaskResult finalResult = TaskResult.builder()
                        ...
                        .outputData(report)
                        .build();
                mathTaskService.saveResult(finalResult);
            ...
            mathTaskService.addLog(task, TaskLog.LogLevel.ERROR, "GRAPH_EXECUTOR", 
    ...
    public static class CreateTaskRequest {
        ...
        public String getResearchGoals() { return researchGoals; }
        public void setResearchGoals(String researchGoals) { this.researchGoals = researchGoals; }
        
        public String getDataRequirements() { return dataRequirements; }
        public void setDataRequirements(String dataRequirements) { this.dataRequirements = dataRequirements; }
        
        public String getModelConstraints() { return modelConstraints; }
        public void setModelConstraints(String modelConstraints) { this.modelConstraints = modelConstraints; }
        ...
        public MathTask.TaskType getType() { return type; }
        public void setType(MathTask.TaskType type) { this.type = type; }
    }
}
